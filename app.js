...
document.addEventListener("click", async function(e) {
    // 1. Kolla om det var knappen som klickades
    if (e.target && e.target.id === "save-workout-btn") {
        e.preventDefault();
        e.stopPropagation();

        console.log("🚀 Spar-knappen fångad via global lyssnare.");

        // 2. Här ligger din exakta spar-logik (kopierad från din kod ovan)
        window.blockAllSync = true;
        ...
        try {
            ...
            setTimeout(() => { window.blockAllSync = false; }, 250);
        }
    }
});
