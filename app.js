...
function filterExercises(category, subtarget = null) {
    ...
    if (plusArea && !document.getElementById('plus-hint-bubble')) {
        ...
        hint.style.cssText = 'position:relative; margin-right:16px;';
...
const div = document.createElement("div");
            ...
            if (exerciseData.sets_data) {
                exerciseData.sets_data.forEach((set, sIdx) => {
                   ...
                   setsHtml += `
                    <div style="display:grid; grid-template-columns: 40px 1fr 1fr 1fr 30px; gap:8px; margin-bottom:8px; align-items:center; opacity: ${showSuccess ? '1' : isCurrent ? '1' : '0.35'}; transition: opacity 0.2s ease;">
