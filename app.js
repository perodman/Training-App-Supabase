    ...
    if (exerciseData.sets_data) {
        exerciseData.sets_data.forEach((set, sIdx) => {
            ...
            <div style="display:grid; grid-template-columns: 40px 1fr 1fr 1fr 30px; gap:8px; margin-bottom:8px; align-items:center; transition: opacity 0.2s ease; position:relative; overflow:visible;">
                ...
                ${isCardio
                    ? `<div style="display:flex; gap:2px; align-items:center;"><input type="text" inputmode="numeric" id="cdm-${exIdx}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; min-width:0; width:100%; opacity:${isCurrent ? '1' : '0.3'}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_min || ''}" placeholder="mm" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${exIdx}, ${sIdx}, 'min')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)"><span style="color:#64748b; font-size:14px; font-weight:700; flex-shrink:0;">:</span><input type="text" inputmode="numeric" id="cds-${exIdx}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; min-width:0; width:100%; opacity:${isCurrent ? '1' : '0.3'}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_sec || ''}" placeholder="ss" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${exIdx}, ${sIdx}, 'sec')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)"></div>`
    ...
    if (exData.sets_data) {
        exData.sets_data.forEach((set, sIdx) => {
            ...
            <div style="display:grid; grid-template-columns: 40px 1fr 1fr 1fr 30px; gap:8px; margin-bottom:8px; align-items:center; transition:opacity 0.2s ease; position:relative; overflow:visible;">
                ...
                ${isCardio
                    ? `<div style="display:flex; gap:2px; align-items:center;">
                        <input type="text" inputmode="numeric" id="cdm-${i}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; min-width:0; width:100%; opacity:${inputOpacity}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_min || ''}" placeholder="mm" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${i}, ${sIdx}, 'min')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)">
        <span style="color:#64748b; font-size:14px; font-weight:700; flex-shrink:0;">:</span>
        <input type="text" inputmode="numeric" id="cds-${i}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; min-width:0; width:100%; opacity:${inputOpacity}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_sec || ''}" placeholder="ss" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${i}, ${sIdx}, 'sec')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)">
                       </div>`
