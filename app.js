...
<div style="display: flex; align-items: center;">
    <div style="width: 160px; background: rgba(34,211,238,0.06); border: 1px solid rgba(34,211,238,0.2); padding: 6px 10px; border-radius: 10px; display: flex; align-items: center; gap: 8px;">
       ...
       <div style="display:flex; gap:12px; flex:1; justify-content:space-around;">
            <div style="display:flex; flex-direction:column; gap:1px;">
                <span style="font-size:8px; color:rgba(34,211,238,0.6); font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">Time</span>
                <span style="color:#fff; font-size:11px; font-weight:700;">${duration || '—'}</span>
            </div>
            <div style="display:flex; flex-direction:column; gap:1px;">
                <span style="font-size:8px; color:rgba(34,211,238,0.6); font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">Dist</span>
                <span style="color:#fff; font-size:11px; font-weight:700;">${s.distance ? s.distance + ' km' : '—'}</span>
            </div>
            <div style="display:flex; flex-direction:column; gap:1px;">
                <span style="font-size:8px; color:rgba(34,211,238,0.6); font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">Pace</span>
                <span style="color:#22d3ee; font-size:11px; font-weight:700;">${pace || '—'}</span>
            </div>
...
function updateCardioTime(inputEl, exIdx, setIdx, field) {
    if (!activeDraft?.data?.[exIdx]?.sets_data?.[setIdx]) return;
    const setObj = activeDraft.data[exIdx].sets_data[setIdx];
    if (field === 'min') setObj.duration_min = inputEl.value;
    if (field === 'sec') setObj.duration_sec = inputEl.value;
    ...
    if (exData.sets_data) {
        exData.sets_data.forEach((set, sIdx) => {
            ...
            <div style="display:grid; grid-template-columns: 40px 1fr 1fr 1fr 30px; gap:8px; margin-bottom:8px; align-items:center; transition:opacity 0.2s ease; position:relative; overflow:visible;">
                ...
                ${isCardio
                    ? `<div style="display:flex; gap:2px; align-items:center;"><input type="text" inputmode="numeric" id="fcdm-${i}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; opacity:${inputOpacity}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_min || ''}" placeholder="min" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${i}, ${sIdx}, 'min')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)"><span style="color:#64748b; font-size:14px; font-weight:700; flex-shrink:0;">:</span><input type="text" inputmode="numeric" id="fcds-${i}-${sIdx}" class="log-input" style="margin:0; padding:8px 4px; font-size:15px; opacity:${inputOpacity}; text-align:center; ${isCurrent ? 'border-color:rgba(245,158,11,0.6);' : ''}" value="${set.duration_sec || ''}" placeholder="sek" ${isLocked ? 'readonly' : ''} oninput="updateCardioTime(this, ${i}, ${sIdx}, 'sec')" onfocus="if(!this.readOnly) handleInputFocus(this)" onblur="if(!this.readOnly) handleInputBlur(this)"></div>`
                ...
                ${isCardio
                    ? `<div id="pace-${i}-${sIdx}" style="display:flex; align-items:center; justify-content:center; font-size:15px; font-weight:800; color:#22d3ee; font-family:monospace;">${calcPace(set.duration, set.distance)}</div>`
                    : (sIdx < exData.sets_data.length - 1
